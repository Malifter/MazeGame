...
public class ShieldGuyEntity extends Entity {
    ...
    public ShieldGuyEntity(Game g, String file, int iX, int iY, int x, int y, int w, int h, Room room) {
    ...
    public void fire(boolean isRight) {
        ...
        if(isRight) {
            shots.add(new ShotEntity(game, "shieldguyshotRight.gif",(int) maxX,
                    (int) minY+8, Face.UP, BLASTER_DAMAGE, AGGRO_RANGE));
        }
        else if(!isRight) {
            shots.add(new ShotEntity(game, "shieldguyshot.gif",(int) minX,
                    (int) minY+8, Face.DOWN, BLASTER_DAMAGE, AGGRO_RANGE));
        }
